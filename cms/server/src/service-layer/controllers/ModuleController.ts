...
import {
  ModulesGetResponse,
  ModuleResponse,
} from "../response-models/ModuleResponse";
import {
  IModule,
  IQuestion,
  IQuiz,
...
export class ModuleController extends Controller {
  ...
  public async reorderModules(
    ...
    const wasReordered =
      await this.moduleService.reorderModules(orderedModuleIds);
    if (!wasReordered) {
      this.setStatus(500);
      return { success: false };
    }
  ...
  public async editSubsection(
    ...
    const result = await this.moduleService.editSubsection(
      subsectionId,
      subsectionChanges,
    );
    return { success: result };
  ...
  public async deleteSubsectionFromModule(
    ...
    const result = await this.moduleService.deleteSubsection(
      moduleId,
      subsectionId,
    );
    return { success: result };
  ...
  public async addQuestion(
    ...
    const newQuestion = await this.moduleService.addQuestionToQuiz(
      quizId,
      questionData,
    );
    if (!newQuestion) {
  ...
  public async editQuestion(
    ...
    const updated = await this.moduleService.updateQuestion(
      questionId,
      questionData,
    );
  ...
  public async deleteQuestion(
    ...
    const wasDeleted = await this.moduleService.deleteQuestion(
      questionId,
      quizId,
    );
  ...
  public async addLink(
    ...
    const success = await this.moduleService.createLink(
      moduleId,
      linkData.title,
      linkData.link,
    );
  ...
  public async editLink(
    ...
    const success = await this.moduleService.updateLink(
      linkId,
      linkData.title,
      linkData.link,
    );
